  ...
  <div>
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="saveRoleSelected">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    async getUserList() {
      ...
      if (res.meta.status !== 200) return this.$messsge.error('获取用户失败')
    ...
    async changeUserStatus(userinfo) {
      ...
      if (res.meta.status !== 200) {
        ...
        return this.$messsge.error('修改用户状态失败')
      }
